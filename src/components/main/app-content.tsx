...
const determineUserRole = (userRolId: number | null): UserRole => {
  console.log("🚀 ~ determineUserRole ~ userRolId:", userRolId)
    ...
    try {
      ...
      const messageListener = (event: MessageEvent) => {
        if (event.data && (event.data.user || event.data.error)) {
            ...
            if (user) {
                console.log("🚀 ~ messageListener ~ user:", user)
                setCurrentUser(user);
                ...
                toast({
                    ...
                    description: `Ha iniciado sesión como ${user.name || user.email} (${determineUserRole(user.email)})`
                });
            }
        }
      };
